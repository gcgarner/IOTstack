...
def main():
  ...
  def preBuild():
    ...
    # Files copy
    shutil.copy(r'%s/influxdb.env' % serviceTemplate, r'%s/influxdb.env' % serviceService)
    return True
  ...
  def checkEnvFiles():
    envFileIssues = []
    if not os.path.exists(serviceTemplate + '/influxdb.env'):
      envFileIssues.append(serviceTemplate + '/influxdb.env does not exist')
