...
def main():
  ...
  def preBuild():
    ...
    # Files copy
    shutil.copy(r'%s/homebridge.env' % serviceTemplate, r'%s/homebridge.env' % serviceService)
    return True
  ...
  def onResize(sig, action):
    global homeBridgeBuildOptions
    global currentMenuItemIndex
    mainRender(1, homeBridgeBuildOptions, currentMenuItemIndex)

  homeBridgeBuildOptions = []
  ...
  def runSelection(selection):
    import types
    global homeBridgeBuildOptions
    if len(homeBridgeBuildOptions[selection]) > 1 and isinstance(homeBridgeBuildOptions[selection][1], types.FunctionType):
      homeBridgeBuildOptions[selection][1]()
    else:
  ...
  def menuEntryPoint():
    ...
    with term.fullscreen():
      menuNavigateDirection = 0
      mainRender(needsRender, homeBridgeBuildOptions, currentMenuItemIndex)
      selectionInProgress = True
      with term.cbreak():
        while selectionInProgress:
          ...
          if menuNavigateDirection != 0: # If a direction was pressed, find next selectable item
            ...
            currentMenuItemIndex = currentMenuItemIndex % len(homeBridgeBuildOptions)
            needsRender = 2

            while not isMenuItemSelectable(homeBridgeBuildOptions, currentMenuItemIndex):
              currentMenuItemIndex += menuNavigateDirection
              currentMenuItemIndex = currentMenuItemIndex % len(homeBridgeBuildOptions)
    return True
...
global currentServiceName # Name of the current service
if currentServiceName == 'homebridge':
  main()
else:
  print("Error. '{}' Tried to run 'homebridge' config".format(currentServiceName))
