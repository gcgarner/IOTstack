...
def main():
  ...
  def onResize(sig, action):
    ...
    global currentMenuItemIndex
    mainRender(1, blynkServerBuildOptions, currentMenuItemIndex)
  ...
  def createMenu():
    ...
    try:
      ...
      pass
    blynkServerBuildOptions.append(["Go back", goBack])
  ...
  def runSelection(selection):
    import types
    global blynkServerBuildOptions
    if len(blynkServerBuildOptions[selection]) > 1 and isinstance(blynkServerBuildOptions[selection][1], types.FunctionType):
      blynkServerBuildOptions[selection][1]()
    else:
  ...
  def menuEntryPoint():
    ...
    with term.fullscreen():
      menuNavigateDirection = 0
      mainRender(needsRender, blynkServerBuildOptions, currentMenuItemIndex)
      selectionInProgress = True
      with term.cbreak():
        while selectionInProgress:
          ...
          if menuNavigateDirection != 0: # If a direction was pressed, find next selectable item
            ...
            currentMenuItemIndex = currentMenuItemIndex % len(blynkServerBuildOptions)
            needsRender = 2

            while not isMenuItemSelectable(blynkServerBuildOptions, currentMenuItemIndex):
              currentMenuItemIndex += menuNavigateDirection
              currentMenuItemIndex = currentMenuItemIndex % len(blynkServerBuildOptions)
    return True
