...
def main():
  ...
  documentationHint = 'https://sensorsiot.github.io/IOTstack/Containers/TasmoAdmin'
  ...
  def onResize(sig, action):
    global tasmoAdminBuildOptions
    global currentMenuItemIndex
    mainRender(1, tasmoAdminBuildOptions, currentMenuItemIndex)

  tasmoAdminBuildOptions = []
  ...
  def mainRender(needsRender, menu, selection):
    ...
    if needsRender == 1:
      ...
      print(term.black_on_cornsilk4(term.center('IOTstack Tasmo Admin Options')))
      print("")
  ...
  def runSelection(selection):
    import types
    global tasmoAdminBuildOptions
    if len(tasmoAdminBuildOptions[selection]) > 1 and isinstance(tasmoAdminBuildOptions[selection][1], types.FunctionType):
      tasmoAdminBuildOptions[selection][1]()
    else:
  ...
  def menuEntryPoint():
    ...
    with term.fullscreen():
      menuNavigateDirection = 0
      mainRender(needsRender, tasmoAdminBuildOptions, currentMenuItemIndex)
      selectionInProgress = True
      with term.cbreak():
        while selectionInProgress:
          ...
          if menuNavigateDirection != 0: # If a direction was pressed, find next selectable item
            ...
            currentMenuItemIndex = currentMenuItemIndex % len(tasmoAdminBuildOptions)
            needsRender = 2

            while not isMenuItemSelectable(tasmoAdminBuildOptions, currentMenuItemIndex):
              currentMenuItemIndex += menuNavigateDirection
              currentMenuItemIndex = currentMenuItemIndex % len(tasmoAdminBuildOptions)
    return True
...
global currentServiceName # Name of the current service
if currentServiceName == 'tasmoadmin':
  main()
else:
  print("Error. '{}' Tried to run 'tasmoadmin' config".format(currentServiceName))
