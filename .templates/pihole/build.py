...
def main():
  ...
  def preBuild():
    ...
    # Files copy
    shutil.copy(r'%s/pihole.env' % serviceTemplate, r'%s/pihole.env' % serviceService)
    return True
  ...
  def checkEnvFiles():
    envFileIssues = []
    if not os.path.exists(serviceTemplate + '/pihole.env'):
      envFileIssues.append(serviceTemplate + '/pihole.env does not exist')
  ...
  def mainRender(needsRender, menu, selection):
    ...
    if needsRender == 1:
      ...
      print(term.black_on_cornsilk4(term.center('IOTstack PiHole Options')))
      print("")
  ...
  def runSelection(selection):
    import types
    global piHoleBuildOptions
    if len(piHoleBuildOptions[selection]) > 1 and isinstance(piHoleBuildOptions[selection][1], types.FunctionType):
      ...
      with term.cbreak():
        while selectionInProgress:
          ...
          if menuNavigateDirection != 0: # If a direction was pressed, find next selectable item
            ...
            currentMenuItemIndex = currentMenuItemIndex % len(piHoleBuildOptions)
            needsRender = 2

            while not isMenuItemSelectable(piHoleBuildOptions, currentMenuItemIndex):
              currentMenuItemIndex += menuNavigateDirection
              currentMenuItemIndex = currentMenuItemIndex % len(piHoleBuildOptions)
    return True
...
global currentServiceName # Name of the current service
if currentServiceName == 'pihole':
  main()
else:
  print("Error. '{}' Tried to run 'pihole' config".format(currentServiceName))
