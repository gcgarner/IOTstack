...
def main():
  ...
  def preBuild():
    ...
    # Files copy
    shutil.copy(r'%s/zigbee2mqtt_assistant.env' % serviceTemplate, r'%s/zigbee2mqtt_assistant.env' % serviceService)
    return True
  ...
  def checkEnvFiles():
    envFileIssues = []
    if not os.path.exists(serviceTemplate + '/zigbee2mqtt_assistant.env'):
      envFileIssues.append(serviceTemplate + '/zigbee2mqtt_assistant.env does not exist')
